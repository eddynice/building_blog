...
if(isset($_POST['submit'])){
    ...
    if (empty($_POST['lastname'])) {
      ...
      (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
          echo "emails must be a valid email address";
          }else{
            $check_email = trim($_POST['email']);
            $check_user_email_sql = "SELECT id from users Where email = 
             ' $check_email'";
      ...
      if (empty($_POST['password'])) {
        echo "passwordis required";
      }elseif (strlen(trim($_POST['password'])) < 6){
        echo "password must br at 6 char";
      }else{
        $password = trim($_POST['password']);
      }

      if (empty($_POST['cpassword'])) {
        echo "cpasswor is required";
      }else {
        $confirm_password = trim($_POST['cpassword']);
        if($password != $confirm_password){
      ...
      $hashed_password  = password_hash($password, PASSWORD_DEFAULT);
      $insert_sql = "INSERT INTO users (firstname, lastname, email, password) VALUES ('$firstname', '$lastname', '$email', ' $hashed_password')";
